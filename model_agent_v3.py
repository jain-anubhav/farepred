"""
Flight price prediction model agent v3 - combined feature groups A-E.

Extends model_v4.py with the following new feature groups:
  A: Price position in market distribution (from training stats at dfd=40)
  B: Market structural features (CV, DFD slope, high-price fraction)
  C: Competitive pressure (duopoly flag, spread, relative price)
  D: Trajectory deviation z-score and percentile
  E: Departure date seasonality (peak summer, peak holiday, etc.)
"""
import pandas as pd
import numpy as np
...
def build_flight_features(df, train_df, global_traj, observation_dfd=OBSERVATION_DFD):
    ...
    # ===========================================================================
    # Group A: Price position in market distribution (from training stats at dfd=40)
    # ===========================================================================
    obs40 = train_df[train_df['dfd'] == 40]
    mkt_price_stats = obs40.groupby('pathod_id')['target_price'].agg(
        ['mean', 'std', 'median']
    ).reset_index()
    mkt_price_stats.columns = ['pathod_id', 'mkt_p40_mean', 'mkt_p40_std', 'mkt_p40_median']
    flight_feats = flight_feats.merge(mkt_price_stats, on='pathod_id', how='left')
    flight_feats['price_zscore_mkt'] = (
        flight_feats['price_at_obs'] - flight_feats['mkt_p40_mean']
    ) / (flight_feats['mkt_p40_std'] + 1e-6)
    flight_feats['price_pctrank_mkt'] = (
        flight_feats['price_at_obs']
        .sub(flight_feats['mkt_p40_mean'])
        .div(flight_feats['mkt_p40_std'] + 1e-6)
        .apply(lambda z: float(scipy_norm.cdf(z)))
    )

    # ===========================================================================
    # Group B: Market structural features (from training data)
    # ===========================================================================
    market_cv = (
        train_df.groupby('pathod_id')['target_price']
        .agg(lambda x: x.std() / (x.mean() + 1e-6))
        .rename('market_price_cv')
        .reset_index()
    )

    mkt_dfd = train_df.groupby(['pathod_id', 'dfd'])['target_price'].median().reset_index()

    def slope(g):
        if len(g) < 2:
            return 0.0
        return float(np.polyfit(g['dfd'].values, g['target_price'].values, 1)[0])

    market_slope = (
        mkt_dfd.groupby('pathod_id')
        .apply(slope)
        .rename('market_dfd_slope')
        .reset_index()
    )

    market_high_pct = (
        train_df.groupby('pathod_id')
        .apply(lambda g: float((g['target_price'] > 1.5 * g['target_price'].median()).mean()))
        .rename('market_high_price_pct')
        .reset_index()
    )

    flight_feats = flight_feats.merge(market_cv, on='pathod_id', how='left')
    flight_feats = flight_feats.merge(market_slope, on='pathod_id', how='left')
    flight_feats = flight_feats.merge(market_high_pct, on='pathod_id', how='left')

    # ===========================================================================
    # Group C: Competitive pressure (from existing columns in flight_feats)
    # ===========================================================================
    flight_feats['is_duopoly'] = (flight_feats['market_n_airlines'] == 2).astype(int)
    flight_feats['airline_price_spread'] = (
        flight_feats['market_max_price'] - flight_feats['market_min_price']
    )
    flight_feats['price_vs_spread_pct'] = (
        (flight_feats['price_at_obs'] - flight_feats['market_min_price']) /
        (flight_feats['airline_price_spread'] + 1e-6)
    ).clip(0, 1)
    flight_feats['airline_rel_price'] = (
        flight_feats['price_at_obs'] / (flight_feats['market_mean_price'] + 1e-6)
    )

    # ===========================================================================
    # Group D: Trajectory deviation z-score (from existing columns)
    # ===========================================================================
    flight_feats['traj_deviation_norm'] = (
        flight_feats['price_at_obs'] - flight_feats['hist_traj_mean_at_obs']
    ) / (flight_feats['hist_traj_std_at_obs'] + 1e-6)
    flight_feats['traj_pctile_at_obs'] = scipy_norm.cdf(
        flight_feats['price_at_obs'],
    ...
    # ===========================================================================
    # Group E: Departure date seasonality
    # ===========================================================================
    dep_date = (
        pd.to_datetime(flight_feats['offer_date'].astype(str)) +
        pd.Timedelta(days=observation_dfd)
    )
    flight_feats['dep_dayofyear'] = dep_date.dt.dayofyear
...
def get_features():
    return [
        ...
        # Group A
        'price_zscore_mkt', 'price_pctrank_mkt',
        # Group B
        'market_price_cv', 'market_dfd_slope', 'market_high_price_pct',
        # Group C
        'is_duopoly', 'airline_price_spread', 'price_vs_spread_pct', 'airline_rel_price',
        # Group D
        'traj_deviation_norm', 'traj_pctile_at_obs',
        # Group E
...
def main():
    ...
    print("Preparing features (offer_date features computed within each dataset)...")
    ...
    # ===========================
    print("\n=== Model A: LightGBM (with all feature groups A-E) ===")
    ...
    rmse_a = evaluate_by_dfd(y_test, pred_a, dfd_test, "LightGBM-v3", baseline_test)

    # ===========================
    # Model B: LightGBM residual
    # ===========================
    print("\n=== Model B: LightGBM Residual (with all feature groups A-E) ===")
    ...
    rmse_b = evaluate_by_dfd(y_test, pred_b, dfd_test, "LightGBM-resid-v3", baseline_test)

    # ===========================
    # Model C: XGBoost
    # ===========================
    print("\n=== Model C: XGBoost (with all feature groups A-E) ===")
    ...
    rmse_c = evaluate_by_dfd(y_test, pred_c, dfd_test, "XGBoost-v3", baseline_test)
    ...
    print("\n--- New feature groups (A-E) importances ---")
    new_feats = [
        'price_zscore_mkt', 'price_pctrank_mkt',
        'market_price_cv', 'market_dfd_slope', 'market_high_price_pct',
        'is_duopoly', 'airline_price_spread', 'price_vs_spread_pct', 'airline_rel_price',
    ...
    new_fi = fi[fi['feature'].isin(new_feats)]
