"""
Flight price prediction model agent v5 - historical price ceiling proximity features.

Based on model_v4.py (offer_date features).

Change from model_v4.py:
  Model B now predicts log_ratio = log((target_price + 1) / (baseline_pred + 1))
  instead of the absolute residual. At inference:
    final_pred = (baseline_pred + 1) * exp(log_ratio_pred) - 1
  This makes the loss proportional — a 10% error on a $200 flight counts the same
  as a 10% error on a $500 flight, reducing high-price error dominance in training.
"""
import pandas as pd
import numpy as np
...
def prepare_data(df, train_df, global_traj, observation_dfd=OBSERVATION_DFD):
    ...
    # === NEW: Historical price ceiling proximity features (iter 5) ===
    data['hist_price_p90_approx'] = data['hist_price_mean'] + 1.28 * data['hist_price_std']
    data['price_to_hist_max_ratio'] = data['price_at_obs'] / (data['hist_price_max'] + 1e-6)
    data['price_to_hist_p90_ratio'] = data['price_at_obs'] / (data['hist_price_p90_approx'] + 1e-6)
    data['hist_price_headroom'] = (data['hist_price_max'] - data['price_at_obs']) / (data['hist_price_std'] + 1e-6)
    data['is_above_hist_max'] = (data['price_at_obs'] > data['hist_price_max']).astype(int)
    data['price_to_hist_max_ratio_sq'] = data['price_to_hist_max_ratio'] ** 2
    data['residual'] = data['target_price'] - data['baseline_pred']

    # Log-ratio target for Model B: log((target_price + 1) / (baseline_pred + 1))
    # This makes errors proportional — a 10% error on a $200 flight counts the same
    # as a 10% error on a $500 flight.
    data['log_ratio'] = np.log((data['target_price'] + 1) / (data['baseline_pred'] + 1))
...
def get_features():
    return [
        ...
        # === NEW: Historical price ceiling proximity features (iter 5) ===
        'hist_price_p90_approx', 'price_to_hist_max_ratio', 'price_to_hist_p90_ratio',
        'hist_price_headroom', 'is_above_hist_max', 'price_to_hist_max_ratio_sq',
    ]
...
def main():
    ...
    print("Preparing features (offer_date + price ceiling proximity features)...")
    ...
    new_feats = ['price_to_hist_max_ratio', 'price_to_hist_p90_ratio', 'hist_price_headroom', 'is_above_hist_max']
    print("  New price ceiling proximity feature missing rates:")
    ...
    # ===========================
    # Model B: LightGBM Log-Ratio
    # Predicts log_ratio = log((target_price + 1) / (baseline_pred + 1))
    # Decoded as: final_pred = (baseline_pred + 1) * exp(log_ratio_pred) - 1
    # ===========================
    print("\n=== Model B: LightGBM-log-ratio ===")
    ...
    pred_b = np.clip(pred_b, 0, baseline_test * 3)  # cap at 3x baseline
    rmse_b = evaluate_by_dfd(y_test, pred_b, dfd_test, "LightGBM-log-ratio", baseline_test)
    ...
    rmse_c = evaluate_by_dfd(y_test, pred_c, dfd_test, "XGBoost-abs", baseline_test)
    ...
    print("\n--- New price ceiling proximity features specifically ---")
    ceiling_fi = fi[fi['feature'].isin(['price_to_hist_max_ratio', 'price_to_hist_p90_ratio', 'hist_price_headroom', 'is_above_hist_max', 'price_to_hist_max_ratio_sq', 'hist_price_p90_approx'])]
    print(ceiling_fi.to_string(index=False))
